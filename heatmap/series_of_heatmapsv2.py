...
def global_normalize(prices, global_min, global_max):
    """Normalize using global dataset range."""
    if global_max > global_min:
        return (prices - global_min) / (global_max - global_min)
    return prices * 0.0
...
def generate_heatmap(df, period, global_min, global_max, out_file):
    subset = df[df["period"] == period]
    if subset.empty:
        return False

    # TAKE LOG TO REDUCE EXTREME PRICE SKEW
    prices_log = np.log1p(subset[PRICE_COL])

    # GLOBAL NORMALIZATION ACROSS ALL PERIODS
    weights = global_normalize(prices_log, global_min, global_max)
...
button { padding: 8px 14px; margin-right: 10px; font-size: 14px; }
...
function next() { if (current < periods.length - 1) { current++; load(); } }
...
def main():
    ...
    # GLOBAL PRICE RANGE (log-transformed)
    price_log = np.log1p(df[PRICE_COL])
    global_min = price_log.min()
    global_max = price_log.max()
    ...
    for period in periods:
        ...
        print(" →", out_file)
        generate_heatmap(df, period, global_min, global_max, out_file)

    print("Building index.html...")
    generate_index_page(periods)

    print("Done! Open index.html to compare periods with consistent color scaling.")
